...
void RF24Network::begin(uint8_t _channel, uint16_t _node_address) {
  #ifdef SERIAL_DEBUG
    Serial.println(F("RF24Network::begin"));
  #endif

  if (!is_valid_address(_node_address)) {
    #ifdef SERIAL_DEBUG
      Serial.println(F("RF24Network::begin Address is not valid"));
    #endif
    return;
  }
  ...
  #ifdef SERIAL_DEBUG
    Serial.print(F("RF24Network::begin node address: "));
    Serial.println(node_address, OCT);
  #endif

  if (!radio.isValid()) {
    delay(2000);
    #ifdef SERIAL_DEBUG
      Serial.println(F("RF24Network::begin !radio.isValid() :("));
    #endif
    return;
  }
  delay(1000);
  ...
  uint8_t i = 6;
#ifdef SERIAL_DEBUG
  Serial.println(F("RF24Network::begin Opening reading pipe:"));
#endif
  ...
  #ifdef SERIAL_DEBUG
    Serial.println(F("RF24Network::begin Start listening"));
  #endif
...
uint8_t RF24Network::update(void) {
  ...
  if (!(networkFlags & FLAG_BYPASS_HOLDS)) {
    ...
    //Serial.println("!(networkFlags & FLAG_BYPASS_HOLDS)");
  #endif
    if ((networkFlags & FLAG_HOLD_INCOMING)
        || (next_frame - frame_queue) + 34 > MAIN_BUFFER_SIZE) {
      #ifdef IF_SERIAL_DEBUG
        //Serial.println("(networkFlags & FLAG_HOLD_INCOMING) || (next_frame - frame_queue) + 34 > MAIN_BUFFER_SIZE");
      #endif
      if (!available()) {
        #ifdef IF_SERIAL_DEBUG
  ...
  while ( radio.isValid() && radio.available(&pipe_num) ){
    ...
    Serial.print(F("During RF24NETWORK update read bytes:"));
...
//                                 *i));
    ...
    if (!is_valid_address(header->to_node)) {
      #if defined (SERIAL_DEBUG)
        Serial.println(F("RF24Network::update not a valid address!!!!!"));
      #endif
    ...
    Serial.println(F("-->node_address"));
  #endif
    if (header->to_node == node_address) {
      #if defined (SERIAL_DEBUG)
          Serial.println("RF24Network::update it is for us!");
      #endif
      if (header->type == NETWORK_PING) {
        #if defined (SERIAL_DEBUG)
          Serial.println("IT IS A PING!");
        #endif
        continue;
      }
      ...
      if (header->type == NETWORK_REQ_ADDRESS && node_address) {
        ...
        #if defined (SERIAL_DEBUG)
            Serial.println(F("NETWORK_REQ_ADDRESS"));
        #endif
      ...
      if ((returnSysMsgs && header->type > 127)
        ...
        /*IF_SERIAL_DEBUG_ROUTING(printf_P(PSTR(
            "%lu MAC: System payload rcvd %d\n"), millis(), returnVal););*/
        #if defined (SERIAL_DEBUG)
            Serial.print(millis());
            Serial.print(F("MAC: System payload rcvd"));
  ...
  if (isFragment) {
    ...
    if (header->type == NETWORK_FIRST_FRAGMENT) {
      ...
      if (header->reserved > (MAX_PAYLOAD_SIZE / max_frame_payload_size)) {
        ...
        printf_P(PSTR(
                     "Frag frame with %d frags exceeds MAX_PAYLOAD_SIZE or out of sequence\n"),
                 header->reserved);
#endif
...
//IF_SERIAL_DEBUG_FRAGMENTATION( Serial.print(F("queue first, total frags ")); Serial.println(header->reserved); );
      ...
      IF_SERIAL_DEBUG_FRAGMENTATION_L2(for (int i = 0;
                                            i < frag_queue.message_size; i++) {
        Serial.println(frag_queue.message_buffer[i],
                       HEX);
      });
    ...
    if (header->type == NETWORK_LAST_FRAGMENT
      ...
      if (MAX_PAYLOAD_SIZE - (next_frame - frame_queue)
        ...
        memcpy(next_frame + 10,
               frag_queue.message_buffer,
               frag_queue.message_size);
      ...
      IF_SERIAL_DEBUG_FRAGMENTATION(printf_P(PSTR(
          "Drop frag payload, queue full\n")););
      return false;
...
uint16_t RF24Network::read(RF24NetworkHeader &header,
                           void *message,
                           uint16_t maxlen) {
  uint16_t bufsize = 0;
...
#if defined RF24NetworkMulticast
/******************************************************************/
bool RF24Network::multicast(RF24NetworkHeader &header,
                            const void *message,
                            uint16_t len,
                            uint8_t level) {
  // Fill out the header
  ...
  header.from_node = node_address;
  #ifdef SERIAL_DEBUG
    Serial.print(F("RF24Network::multicast header: "));
    Serial.println(header.toString());
    Serial.print(F("RF24Network::multicast message size: "));
    Serial.print(len);
    Serial.print(F(" to level: "));
    Serial.print(level);
    Serial.print(F(" LevelToAddress: "));
    Serial.println(levelToAddress(level), OCT);
  #endif
  return write(header, message, len, levelToAddress(level));
}
/******************************************************************/
bool RF24Network::multicastNoFlag(RF24NetworkHeader &header,
                            const void *message,
                            uint16_t len,
                            uint8_t level) {
  // Fill out the header
  header.to_node = (0100 * static_cast<uint16_t >(level));
  header.from_node = node_address;
  #ifdef SERIAL_DEBUG
  Serial.print(F("RF24Network::multicast header: "));
  Serial.println(header.toString());
...
bool RF24Network::write(RF24NetworkHeader &header,
...
#ifdef SERIAL_DEBUG
  Serial.println(F("RF24Network::write (0) "));
#endif
  ...
  delayMicroseconds(200);
    ...
    #ifdef SERIAL_DEBUG
      Serial.print(F("RF24Network::write (0) header:"));
      Serial.println(header.toString());
      Serial.print(F("RF24Network::write (0) len:"));
      Serial.print(len);
      Serial.print(F(" max_frame_payload_size: "));
      Serial.println(max_frame_payload_size);
    #endif
    ...
    if (len > MAX_PAYLOAD_SIZE) {
      IF_SERIAL_DEBUG(printf(
          "%u: NET write message failed. Given 'len' %d is bigger than the MAX Payload size %i\n\r",
          millis(),
          len,
          MAX_PAYLOAD_SIZE););
      return false;
    ...
    uint8_t fragment_id = (len % max_frame_payload_size != 0) + ((len)
        / max_frame_payload_size);  //the number of fragments to send = ceil(len/max_frame_payload_size)
    ...
    while (fragment_id > 0) {
      ...
      if (fragment_id == 1) {
        header.type =
            NETWORK_LAST_FRAGMENT;  //Set the last fragment flag to indicate the last fragment
        header.reserved =
        ...
        if (msgCount == 0) {
          ...
          header.type =
              NETWORK_MORE_FRAGMENTS; //Set the more fragments flag to indicate a fragmented frame
        }
      }
      ...
      if (!ok && retriesPerFrag >= 3) {
        IF_SERIAL_DEBUG_FRAGMENTATION(printf(
            "%lu: FRG TX with fragmentID '%d' failed after %d fragments. Abort.\n\r",
            millis(),
            fragment_id,
            msgCount););
        break;
    ...
    IF_SERIAL_DEBUG_FRAGMENTATION(printf(
        "%u: FRG total message fragments sent %i. \n",
        millis(),
        msgCount););
    if (fragment_id > 0) {
      txTime = millis();
...
bool RF24Network::_write(RF24NetworkHeader &header,
                         const void *message,
                         uint16_t len,
                         uint16_t writeDirect) {
  // Fill out the header
  ...
  # if defined(SERIAL_DEBUG)
    Serial.println(F("RF24Network::_write (1) "));
    Serial.print(millis());
    Serial.print(F(": NET Sending "));
    Serial.println(header.toString());
    Serial.print(F("Len of message: "));
    Serial.print(len);
    Serial.print(F(" writeDirect:(OCT) "));
    Serial.print(writeDirect, OCT);
    Serial.print(F(" writeDirect:(DEC) "));
    Serial.print(writeDirect);
    Serial.print(F(" writeDirect:(HEX) "));
    Serial.println(writeDirect, HEX);
  #endif
  ...
  if (len) {
   ...
   /*  IF_SERIAL_DEBUG(uint16_t tmpLen = len;
                        printf_P(PSTR("%lu: NET message "), millis());
                        const uint8_t *charPtr =
                            reinterpret_cast<const uint8_t *>(message);
                        while (tmpLen--) { printf("%02x ", charPtr[tmpLen]); }
                        printf_P(PSTR("\n\r")));
    #endif
*/
  }
  ...
  if (writeDirect != 070) {
    ...
    #endif
    uint8_t sendType =
        USER_TX_TO_LOGICAL_ADDRESS; // Payload is multicast to the first node, and routed normally to the next

    /* if (header.to_node == 0100) {
      #ifdef SERIAL_DEBUG
        Serial.println(F("RF24Network::_write (1) header type became USER_TX_MULTICAST"));
      #endif
      sendType = USER_TX_MULTICAST;
    }*/
    if ((header.to_node | 0700) == 0700) {
      #ifdef SERIAL_DEBUG
        Serial.println(F("RF24Network::_write (1) header type became USER_TX_MULTICAST_NOFLAG"));
      #endif
      header.to_node = 0100;
      sendType = USER_TX_MULTICAST_NOFLAG;
    }
    else if (header.to_node == writeDirect) {
      #ifdef SERIAL_DEBUG
        Serial.println(F("RF24Network::_write (1) header type became USER_TX_TO_PHYSICAL_ADDRESS"));
      #endif
      sendType =
          USER_TX_TO_PHYSICAL_ADDRESS; // Payload is multicast to the first node, which is the recipient
    }
    #ifdef SERIAL_DEBUG
      Serial.print(F("RF24Network::write (1) write(writeDirect= "));
      Serial.print(writeDirect);
      Serial.print(F(", sendType= "));
  ...
  #ifdef SERIAL_DEBUG
    Serial.print(F("RF24Network::write (1) write(header.to_node= "));
    Serial.print(header.to_node, OCT);
    Serial.print(F(", TX_NORMAL= "));
    Serial.print(TX_NORMAL);
    Serial.println(F(" )"));
  #endif
                        ...
                        uint8_t directTo)  // Direct To: 0 = First Payload, standard routing, 1=routed payload, 2=directRoute to host, 3=directRoute to Route
{
  ...
  #ifdef SERIAL_DEBUG
    Serial.println(F("RF24Network::write (2)"));
  #endif
  if (frame_buffer[6] > 64 && frame_buffer[6] < 192) {
    isAckType = true;
    #if defined (SERIAL_DEBUG)
      Serial.println(F("RF24Network::write ACK requested for this frame"));
    #endif
  }
  ...
  if (!is_valid_address(to_node)) {
    #if defined (SERIAL_DEBUG)
      Serial.println(F("RF24Network::write !is_valid_address(to_node) returning false"));
    #endif
    return false;
  }
  #if defined (SERIAL_DEBUG)
    Serial.print(F("RF24Network::write to_node= "));
    Serial.print(to_node, OCT);
    Serial.print(F(" direct_to: "));
    Serial.println(directTo, OCT);
  #endif
...
//                           millis(),
//                           to_node,
//                           conversion.send_node,
//                           conversion.send_pipe));
    #if defined (SERIAL_DEBUG)
      Serial.print(millis());
      Serial.print(F(": MAC Sending to "));
      Serial.print(to_node, OCT);
      Serial.print(F(" via "));
      Serial.print(conversion.send_node, OCT);
      Serial.print(F(" on pipe "));
      Serial.println(conversion.send_pipe, HEX);
    #endif

  #endif

  /**Write it*/
  ok = write_to_pipe(conversion.send_node,
                     conversion.send_pipe,
                     conversion.multicast, directTo == USER_TX_MULTICAST_NOFLAG);

  #if defined (SERIAL_DEBUG)
    Serial.println("Wrote! i hope");
  #endif
  if (!ok) {
    #if defined (RF24_LINUX)
    ...
    #else
      //    IF_SERIAL_DEBUG_ROUTING(printf_P(PSTR(
      //        "%lu: MAC Send fail to 0%o via 0%o on pipe %x\n\r"),
      //                                     millis(),
      //                                     to_node,
      //                                     conversion.send_node,
      //                                     conversion.send_pipe););
        ...
        Serial.print(millis());
        Serial.print(F(": MAC Send fail to "));
        Serial.print(to_node, OCT);
        Serial.print(F(" via "));
        Serial.print(conversion.send_node, OCT);
        Serial.print(F(" on pipe "));
  ...
  if (directTo == TX_ROUTED && ok && conversion.send_node == to_node
      ...
      Serial.print(millis());
      Serial.println(F(": directTo == TX_ROUTED && ok && conversion.send_node == to_node "
                           "&& isAckType"));
    #endif
    RF24NetworkHeader *header = (RF24NetworkHeader *) &frame_buffer;
    header->type =
        NETWORK_ACK;            // Set the payload type to NETWORK_ACK
    header->to_node =
    ...
    write_to_pipe(conversion.send_node,
                  conversion.send_pipe,
    ...
    #else
//  IF_SERIAL_DEBUG_ROUTING(printf_P(PSTR(
//      "%lu MAC: Route OK to 0%o ACK sent to 0%o\n"),
//                                   millis(),
//                                   to_node,
//                                   header->from_node););
      #if defined (SERIAL_DEBUG)
        Serial.print(millis());
        Serial.print(F(": Route OK to "));
        Serial.print(to_node, OCT);
  ...
  if (ok && conversion.send_node != to_node && (directTo == 0 || directTo == 3)
    ...
    // Now, continue listening
      ...
      Serial.print(millis());
      Serial.println(F("ok && conversion.send_node != to_node && (directTo == 0 || directTo == 3) "
                         "&& isAckType"));
    #endif
    ...
    while (update() != NETWORK_ACK) {
      ...
      if (millis() - reply_time > routeTimeout) {
        ...
        #else
//        IF_SERIAL_DEBUG_ROUTING(printf_P(PSTR(
//            "%lu: MAC Network ACK fail from 0%o via 0%o on pipe %x\n\r"),
//                                         millis(),
//                                         to_node,
//                                         conversion.send_node,
//                                         conversion.send_pipe););
          #if defined (SERIAL_DEBUG)
                Serial.print(millis());
                Serial.print(F(":MAC Network ACK fail from "));
                Serial.print(to_node, OCT);
                Serial.print(F(" via "));
                Serial.print(conversion.send_node, OCT);
                Serial.print(F(" on pipe "));
  ...
  if (!(networkFlags & FLAG_FAST_FRAG)) {
      ...
      // Now, continue listening
      #ifdef SERIAL_DEBUG
        Serial.println(F("RF24Network startListening"));
      #endif
      radio.startListening();
    #endif
  }
...
bool RF24Network::logicalToPhysicalAddress(logicalToPhysicalStruct *conversionInfo) {
  ...
  if (*directTo > TX_ROUTED) {
    ...
    if(*directTo == USER_TX_MULTICAST_NOFLAG)
      *multicast = 0;
    else
      *multicast = 1;
    //if(*directTo == USER_TX_MULTICAST || *directTo == USER_TX_TO_PHYSICAL_ADDRESS){
    #if defined (SERIAL_DEBUG)
      Serial.println("RF24Network::logicalToPhysicalAddress Multicast so pipe 0");
    #endif
    ...
    #if defined (SERIAL_DEBUG)
      Serial.println("RF24Network::logicalToPhysicalAddress is_direct_child so pipe 5");
    #endif
    // Send directly
    pre_conversion_send_node = *to_node;
    ...
    #if defined (SERIAL_DEBUG)
        Serial.println("RF24Network::logicalToPhysicalAddress is_descendant so pipe 5");
    #endif
  ...
  *directTo = pre_conversion_send_pipe;
  #if defined (SERIAL_DEBUG)
    Serial.print(F("RF24Network::logicalToPhysicalAddress to_node: "));
    Serial.print(*to_node, OCT);
    Serial.print(F(" directTo: "));
    Serial.println(*directTo, OCT);
  #endif
  return 1;
...
bool RF24Network::write_to_pipe(uint16_t node, uint8_t pipe, bool multicast, bool disableack) {
  bool ok = false;
  uint64_t out_pipe = pipe_address(node, pipe);

  #if defined (SERIAL_DEBUG)
    Serial.print(F("RF24Network::write_to_pipe to node: "));
    Serial.print(node, OCT);
    Serial.print(F(" via pipe: 0x"));
    for(int i = 5; i >= 0; i--)
      Serial.print(reinterpret_cast<uint8_t *>(&out_pipe)[i], HEX);
    Serial.print(F(" multicast: "));
    Serial.println(multicast? "YES":"NO");
  #endif
  ...
  if (multicast || disableack ) { radio.setAutoAck(0, 0); } else { radio.setAutoAck(0, 1); }

  radio.openWritingPipe(out_pipe);

  ok = radio.writeFast(frame_buffer, frame_size, multicast);
#if defined (SERIAL_DEBUG)
  Serial.print(F("RF24Network::write_to_pipe ok: "));
  Serial.println(ok? "YES":"NO");
#endif
  if (!(networkFlags & FLAG_FAST_FRAG)) {
    #if defined (SERIAL_DEBUG)
      Serial.println(F("RF24Network::write_to_pipe !(networkFlags & FLAG_FAST_FRAG) "));
    #endif
    ok = radio.txStandBy(txTimeout);
    radio.setAutoAck(0, 0);
  }
...
*/
#if defined (SERIAL_DEBUG)
  Serial.print(F("RF24Network::write_to_pipe second chance ok: "));
  Serial.println(ok? "YES":"NO");
#endif
  return ok;
}
...
const char *RF24NetworkHeader::toString(void) const {
  ...
  sprintf_P(buffer,
            PSTR("id %u from 0%o to 0%o type %d"),
            id,
            from_node,
            to_node,
            type);
...
#ifdef SERIAL_DEBUG
  Serial.print(F("RF24Network::setup_address node= "));
  Serial.print(node_address, OCT);
  Serial.print(F(" mask= "));
  Serial.print(node_mask, OCT);
  Serial.print(F(" parent= "));
  Serial.print(parent_node, OCT);
  Serial.print(F(" parentPipe= "));
  Serial.println(parent_pipe, OCT);
  #if defined (RF24NetworkMulticast)
    Serial.println(F("RF24Network::setup_address Multicast is Enabled"));
  #endif
#endif
...
bool RF24Network::is_valid_address(uint16_t node) {
  ...
  while (node) {
    ...
    {
      result = false;
      IF_SERIAL_DEBUG_MINIMAL(printf_P(PSTR(
          "*** WARNING *** Invalid address 0%o\n\r"), node););
      break;
    }
    node >>= 3;
  }
...
uint64_t pipe_address(uint16_t node, uint8_t pipe) {
...
#ifdef SERIAL_DEBUG
  //Serial.print(F("RF24Network::pipe_address initially is: 0x"));
  //for(int i = 5; i >= 0; i--)
  ...
  uint16_t dec = node;
  #ifdef SERIAL_DEBUG
    Serial.print(F("RF24Network::pipe_address dec: "));
    Serial.println(dec, OCT);
    Serial.print(F("initial pipe: "));
    for(int i = 5; i >= 0; i--)
      Serial.print(out[i], HEX);
  #endif
  while (dec) {
    #if defined (RF24NetworkMulticast)
      #ifdef SERIAL_DEBUG
      //  Serial.println(F("RF24Network::pipe_address RF24NetworkMulticast is enabled doing if"));
      #endif
      if (pipe != 0 || !node) {
      #ifdef SERIAL_DEBUG
      //    Serial.println(F("RF24Network::pipe_address pipe Mulficast modification (pipe!= 0 || !node)"));
      #endif
    #endif
        out[count] = address_translation[(dec
          % 8)];// Convert our decimal values to octal, translate them to address bytes, and set our address
      #ifdef SERIAL_DEBUG
        //Serial.print(F("RF24Network::pipe_address count: "));
        //Serial.print(count);
        //Serial.print(F(" out["));
        //Serial.print(count);
        //Serial.print(F(")]: "));
        //Serial.print(out[count], HEX);
        //Serial.print(F(" address has become: 0x"));
        Serial.print(F(" -> 0x"));
        for(int i = 5; i >= 0; i--)
          Serial.print(out[i], HEX);
      #endif
        ...
        #ifdef SERIAL_DEBUG
          //Serial.print(F("RF24Network::pipe_address Skipping pipe:"));
          //Serial.print(pipe);
          //Serial.print(F(" node:"));
          //Serial.println(node);
        #endif
  ...
  if (pipe != 0 || !node) {
    #ifdef SERIAL_DEBUG
      //Serial.println(F("RF24Network::pipe_address multicast modification (pipe != 0 || !node)"));
    #endif
#endif
    out[0] = address_translation[pipe];
    #ifdef SERIAL_DEBUG
      //Serial.print(F("RF24Network::pipe_address  out[0]: "));
    ...
    #ifdef SERIAL_DEBUG
      //Serial.print(F("RF24Network::pipe_address else case out[1]: "));
      //Serial.print(out[1], HEX);
      Serial.print(F(" else[1] -> 0x"));
      for(int i = 5; i >= 0; i--)
        Serial.print(out[i], HEX);
      Serial.println();
    #endif
...
//                      printf_P(PSTR(
//                          "%lu: NET Pipe %i on node 0%o has address %lx%x\n\r"),
//                               millis(),
//                               pipe,
//                               node,
//                               *top,
//                               *out));
  #ifdef  SERIAL_DEBUG
    Serial.print(millis());
    Serial.print(F(": NET Pipe "));
    Serial.print(pipe);
    Serial.print(F(" on node "));
    Serial.print(node, OCT);
    Serial.print(F(" has address 0x"));
    for(int i = 5; i >= 0; i--)
      Serial.print(out[i], HEX);
    Serial.print(F(" sizeof(result): "));
